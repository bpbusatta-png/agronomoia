...
- **Testes automatizados**: backend tem suíte pytest (39 testes, `backend/tests/`) cobrindo login/refresh JWT, RBAC do núcleo organizacional, gate de validação humana de plantas atípicas, upload de arquivos e reconhecimento por IA (mockado, sem depender do MinIO/Gemini rodando) — roda contra banco Postgres real separado (`agronomo_ia_test`) com isolamento por rollback de transação. Dashboard tem suíte Vitest + React Testing Library (30 testes, `frontend/dashboard/src/**/*.test.tsx`) cobrindo autenticação, rota protegida, o motor genérico de CRUD, o fluxo de validação humana e o reconhecimento por IA — API mockada, não depende do backend rodando. Mobile tem suíte Jest + React Native Testing Library (44 testes, `mobile/app-campo/src/**/*.test.ts(x)`) cobrindo `db.ts`, `sync.ts` (incluindo o retry pendente+erro), `api.ts` (interceptors via `axios-mock-adapter`), `AuthContext` e duas telas (`ColheitaScreen`, `ReconhecimentoScreen`) — `Platform.OS` forçado para `'web'` no setup para reaproveitar o caminho já validado via Expo web, sem mockar módulos nativos. Ver seções "Testes automatizados" no `README.md`, `frontend/dashboard/README.md` e `mobile/app-campo/README.md`.
