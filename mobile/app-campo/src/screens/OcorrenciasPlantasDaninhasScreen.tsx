...
import {
  ...
  getTalhoes,
  listQueue,
  type PlantaDaninhaCatalogoCache,
  type QueueItem,
  type TalhaoCache,
} from '../lib/db'
import { syncPendingOcorrenciasPlantasDaninhas, type OcorrenciaPlantaDaninhaPayload } from '../lib/sync'

function today(): string {
  return new Date().toISOString().slice(0, 10)
}

export function OcorrenciasPlantasDaninhasScreen() {
  const [talhoes, setTalhoes] = useState<TalhaoCache[]>([])
  const [plantasDaninhas, setPlantasDaninhas] = useState<PlantaDaninhaCatalogoCache[]>([])
  const [ocorrencias, setOcorrencias] = useState<QueueItem<OcorrenciaPlantaDaninhaPayload>[]>([])
  const [selectedTalhao, setSelectedTalhao] = useState<TalhaoCache | null>(null)
  const [selectedPlantaDaninha, setSelectedPlantaDaninha] = useState<PlantaDaninhaCatalogoCache | null>(null)
  ...
  const [plantaDaninhaPickerVisible, setPlantaDaninhaPickerVisible] = useState(false)
  ...
  const reload = useCallback(async () => {
    const [t, p, o] = await Promise.all([
      getTalhoes(),
      getCache<PlantaDaninhaCatalogoCache>('plantas_daninhas_catalogo'),
      listQueue<OcorrenciaPlantaDaninhaPayload>('ocorrencias_plantas_daninhas'),
    ])
    setTalhoes(t)
    setPlantasDaninhas(p)
    setOcorrencias(o)
  }, [])
  ...
  async function handleSalvar() {
    ...
    if (!selectedPlantaDaninha) {
      setSaveError('Selecione uma planta daninha')
      return
    }
    await addToQueue<OcorrenciaPlantaDaninhaPayload>('ocorrencias_plantas_daninhas', {
      talhao_id: selectedTalhao.id,
      talhao_codigo: selectedTalhao.codigo,
      planta_daninha_id: selectedPlantaDaninha.id,
      planta_daninha_nome: selectedPlantaDaninha.nome_comum,
      nivel_infestacao: nivelInfestacao || null,
      estadio_cultura: estadioCultura || null,
      data,
    })
    setSelectedTalhao(null)
    setSelectedPlantaDaninha(null)
    setNivelInfestacao('')
    setEstadioCultura('')
    setData(today())
    await reload()
  ...
  async function handleSincronizar() {
    ...
    setSyncMessage(null)
    try {
      const result = await syncPendingOcorrenciasPlantasDaninhas()
      setSyncMessage(
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Nova ocorrência de planta daninha</Text>

      <Text style={styles.label}>Talhão</Text>
      <Pressable
        style={styles.selectBox}
        onPress={() => setTalhaoPickerVisible(true)}
        testID="talhao-select-planta-daninha"
      >
      ...
      <Text style={styles.label}>Planta daninha</Text>
      <Pressable
        style={styles.selectBox}
        onPress={() => setPlantaDaninhaPickerVisible(true)}
        testID="planta-daninha-select"
      >
        <Text style={selectedPlantaDaninha ? styles.selectText : styles.selectPlaceholder}>
          {selectedPlantaDaninha ? selectedPlantaDaninha.nome_comum : 'Selecione...'}
        </Text>
      </Pressable>

      <Text style={styles.label}>Nível de infestação</Text>
      <TextInput style={styles.input} value={nivelInfestacao} onChangeText={setNivelInfestacao} />
      ...
      <Pressable style={styles.saveButton} onPress={handleSalvar} testID="salvar-planta-daninha-button">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            <Text style={styles.itemLine}>Planta daninha: {item.payload.planta_daninha_nome}</Text>
            {item.payload.nivel_infestacao && (
              <Text style={styles.itemLine}>Nível de infestação: {item.payload.nivel_infestacao}</Text>
            )}
          </View>
        )}
      ...
      <CachePickerModal
        visible={plantaDaninhaPickerVisible}
        title="Selecione a planta daninha"
        items={plantasDaninhas}
        labelFor={(item) => item.nome_comum}
        onSelect={(item) => {
          setSelectedPlantaDaninha(item)
          setPlantaDaninhaPickerVisible(false)
        }}
        onClose={() => setPlantaDaninhaPickerVisible(false)}
      />
    </View>
  )
}
