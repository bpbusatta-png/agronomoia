...
import { syncPendingAnalisesSolo, type AnaliseSoloPayload } from '../lib/sync'

function today(): string {
  return new Date().toISOString().slice(0, 10)
}

export function AnalisesSoloScreen() {
  const [talhoes, setTalhoes] = useState<TalhaoCache[]>([])
  const [analises, setAnalises] = useState<QueueItem<AnaliseSoloPayload>[]>([])
  ...
  const reload = useCallback(async () => {
    const [t, a] = await Promise.all([getTalhoes(), listQueue<AnaliseSoloPayload>('analises_solo')])
    setTalhoes(t)
    setAnalises(a)
  }, [])
  ...
  async function handleSalvar() {
    ...
    await addToQueue<AnaliseSoloPayload>('analises_solo', {
      talhao_id: selectedTalhao.id,
      talhao_codigo: selectedTalhao.codigo,
      data,
      ph: ph || null,
      materia_organica: materiaOrganica || null,
    })
  ...
  async function handleSincronizar() {
    ...
    setSyncMessage(null)
    try {
      const result = await syncPendingAnalisesSolo()
      setSyncMessage(
  ...
  const pendentesCount = analises.filter((a) => a.status === 'pendente' || a.status === 'erro').length

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Nova análise de solo</Text>

      <Text style={styles.label}>Talhão</Text>
      <Pressable style={styles.selectBox} onPress={() => setPickerVisible(true)} testID="talhao-select-solo">
      ...
      <TextInput style={styles.input} value={ph} onChangeText={setPh} keyboardType="numeric" testID="ph-input" />

      <Text style={styles.label}>Matéria orgânica (%)</Text>
      <TextInput style={styles.input} value={materiaOrganica} onChangeText={setMateriaOrganica} keyboardType="numeric" />

      {saveError && <Text style={styles.error}>{saveError}</Text>}

      <Pressable style={styles.saveButton} onPress={handleSalvar} testID="salvar-solo-button">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            {item.payload.ph && <Text style={styles.itemLine}>pH: {item.payload.ph}</Text>}
          </View>
        )}
        ListEmptyComponent={<Text style={styles.empty}>Nenhuma análise registrada ainda</Text>}
      />
