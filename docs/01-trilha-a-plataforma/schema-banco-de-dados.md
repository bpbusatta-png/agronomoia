...
*Item A2 · Trilha A — Plataforma Central · pasta sugerida: `01-trilha-a-plataforma/`*

Schema relacional em PostgreSQL, com a extensão **PostGIS** para os campos geoespaciais (`geometry`). Se a stack escolhida for outra (MySQL, SQL Server), os tipos `geometry`/`JSONB` precisam de adaptação equivalente.

## Convenções
- Toda tabela tem `id` (UUID, chave primária)
- Chaves estrangeiras seguem o padrão `<tabela>_id`
- Campos geográficos: `Point` para localização única, `Polygon` para contorno de talhão
- Datas de safra seguem o padrão `'2025/2026'`
...
> Nota de ordem de criação: `modelos_versoes` é referenciada por `ocorrencias_pragas`, `ocorrencias_doencas` e `plantas_atipicas_ocorrencias` — crie-a antes dessas tabelas, ou adicione as constraints de FK depois com `ALTER TABLE`.
...
| De | Para | Tipo |
|---|---|---|
| Cooperado | Fazenda | 1 → N |
| Fazenda | Talhão | 1 → N |
| Talhão | Cultivar, Empresa, Safra | N → 1 |
| Talhão | Inspeções, Aplicações, Fotografias, NDVI, Produtividade, Colheita | 1 → N |
| Fotografia | Ocorrência de Praga/Doença/Planta Atípica | 1 → 1 (por análise) |
| Ocorrência (qualquer) | Validação Humana | N → 1 |
| Contrato | Empresa, Cultivar, Cooperado, Safra | N → 1 |
